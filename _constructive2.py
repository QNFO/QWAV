"""Constructive barrier proof d=1..10 with explicit flushing."""
import sys
...
# Write results to file to avoid buffering issues
with open(r'G:\My Drive\QWAV\_barrier_out.txt', 'w') as out:
    out.write("Constructive barrier proofs (d=1..10):\n")
    all_ok = True
    for d in range(1, 11):
        B = 2**d
        ...
        line = f"  d={d:>2}  B={B:>6}  flip={ry} noflip={rn}  {status}\n"
        out.write(line)
    
    out.write(f"\nOverall: {'ALL PASSED' if all_ok else 'FAILURES DETECTED'}\n")

print("Results written to _barrier_out.txt")
